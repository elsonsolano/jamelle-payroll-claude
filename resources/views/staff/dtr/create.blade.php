<x-staff-layout title="Log DTR" :hide-header="true">
...
<div class="-mx-4 -mt-4">
    <div class="flex items-center justify-between gap-2 px-4 py-3 bg-green-500">
        ...
        <div class="text-right">
            <div class="text-[15px] font-bold text-white leading-tight">Log DTR</div>
...
<script>
function dtrCreateForm() {
    return {
        hasOt: {{ old('has_ot') ? 'true' : 'false' }},
    };
}
</script>

<form id="dtr-form" method="POST" action="{{ route('staff.dtr.store') }}" x-data="dtrCreateForm()">
@csrf
...
<div class="flex flex-col gap-[14px] pt-4">
    ...
    <div>
        ...
        <div class="grid grid-cols-2 gap-2">
            ...
            @foreach([
                ['label' => 'Time In',     'name' => 'time_in'],
                ['label' => 'Start Break', 'name' => 'am_out'],
                ['label' => 'End Break',   'name' => 'pm_in'],
                ['label' => 'Time Out',    'name' => 'time_out'],
            ] as $p)
    ...
    <div>
        ...
        <textarea name="notes" rows="3" maxlength="500"
                  ...
                  style="border-width:1.5px; border-color:#d9ddd6; box-shadow:0 1px 2px rgba(15,20,16,.04); min-height:70px;">{{ old('notes') }}</textarea>
